...
def generate_random_obstacles(X, start, end, n):
    ...
    It also respects start and end points so that they don't
    lie inside of an obstacle.
    """
    ...
    while i < n:
        ...
        for j in range(X.dimensions):
            # None of the sides of a hyperrectangle can be higher than 0.1
            # of the total span in that particular X.dimensions
            max_edge_length = (
                X.dimension_lengths[j][1] - X.dimension_lengths[j][0]) / 10.0
            # None of the sides of a hyperrectangle can be higher than 0.01
            # of the total span in that particular X.dimensions
            min_edge_length = (
                X.dimension_lengths[j][1] - X.dimension_lengths[j][0]) / 100.0
        ...
        # Check newly generated obstacle intersects any former ones.
        # Also respect start and end points
        if len(list(X.obs.intersection(obstacle))) > 0\
                or scollision or fcollision:
            continue
        i += 1
